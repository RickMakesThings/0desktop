...
recents=-1
while read -r ; do
(( ++recents )) ; done < ~/.config/rofi/data/unicode-recent.txt
...
while read recent; do
        ...
        coproc nerdfont ( xdotool key Escape
                          sed -i /$recent/d  ~/.config/rofi/data/unicode-recent.txt  > /dev/null  2>&1
                          sed -i "1s/^/$recent\n/" ~/.config/rofi/data/unicode-recent.txt  > /dev/null  2>&1
                          sleep .05
                          xdotool type --clearmodifiers $recent )  
        exit 0
        fi
done < ~/.config/rofi/data/unicode-recent.txt

# Get the emoji list from a file | create entry for each line
while read code description; do
        echo -en "$code\0meta\x1f${description}\x1ficon\x1f<span color='#727272' font='RobotoMono Nerd Font #Mono'>\\U$code</span>\n"
        # On entry accept
        if [ x"$@" = x"$code" ]; then
        ...
        coproc nerdfont ( xdotool key Escape
                          eval sed -i /$(echo -en "\\U$code")/d  ~/.config/rofi/data/unicode-recent.txt  #> /dev/null  2>&1
                          eval sed -i "1i$(echo -en \\U$code)" ~/.config/rofi/data/unicode-recent.txt  #> /dev/null  2>&1
                          sleep .05
                          eval xdotool type --clearmodifiers $( echo -en "\\U$code" )) 
        exit 0
        fi
done < ~/.config/rofi/data/unicode.txt
